...
/**
 * Лента чипов под шапкой: быстрые точки входа, а не полный индекс.
 *
 * Все тринадцать секторов в одну строку не помещаются. Прокрутка их не
 * спасает: на macOS полоса прокрутки накладная, места не занимает и мышью
 * за неё не взяться, так что половина секторов оставалась недостижимой.
 * Перенос на вторую строку съедал первый экран. Поэтому здесь шесть самых
 * нагруженных секторов и ссылка на полный список — полный индекс живёт
 * в подвале, на хабе трекера и в сайдбаре, то есть тоже на каждой странице.
 */
export const CHIP_RAIL = [
  ...sectors.slice(0, 6).map((s) => ({ label: s.label, href: ROUTES.sector(s.slug) })),
  { label: `All ${sectors.length} sectors`, href: ROUTES.tracker(), all: true },
];
